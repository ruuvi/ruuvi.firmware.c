                                      ...
                                      ruuvi_interface_communication_radio_activity_evt_t evt)
{
  ...
  {
    // Sample
    if((APPLICATION_ADC_SAMPLE_INTERVAL_MS < ruuvi_interface_rtc_millis() - t_sample
        || 0 == t_sample))
    {
...
ruuvi_driver_status_t task_adc_init(void)
{
  ...
  ruuvi_interface_timer_mode_t mode = APPLICATION_BATTERY_VOLTAGE_SIMPLE ?
                                      RUUVI_INTERFACE_TIMER_MODE_REPEATED : RUUVI_INTERFACE_TIMER_MODE_SINGLE_SHOT;
